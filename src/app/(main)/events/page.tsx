    ...
    <div className="mx-auto max-w-[1240px] px-[26px] pb-[104px] pt-[34px]">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3.5">
        <div>
          ...
          <div className="text-[13.5px] text-dim">{has ? `${list.length} event${list.length === 1 ? '' : 's'} · hosting ${list.length}` : 'No events yet'}</div>
        </div>
        <div className="flex items-center gap-2.5">
          <div className="flex h-[34px] w-[220px] items-center gap-2 rounded-[9px] border border-border bg-s1 px-3">
            ...
            <span className="text-[13.5px] text-faint">Search events…</span>
          </div>
          <span className="flex h-[34px] cursor-pointer items-center gap-1.5 rounded-[9px] border border-border bg-s1 px-2.5 text-[13px] text-dim">Closest date <ChevronDown size={15} className="text-dim" /></span>
        </div>
      </div>

      <div className="mb-4 flex flex-wrap items-center gap-2">
        {FILTERS.map((f, i) => (
          <span key={f} className={`flex h-[30px] cursor-pointer items-center rounded-full px-[13px] text-[13px] ${i === 0 ? 'bg-accent font-semibold text-on-accent' : 'border border-border bg-s1 font-medium text-dim'}`}>{f}</span>
        ))}
      </div>
      ...
      <div className="mb-3 mt-7 flex items-center justify-between">
        <div className="flex items-center gap-2 text-[14.5px] font-semibold">
          ...
          <span className="rounded-full border border-border bg-s2 px-[7px] py-px text-[12px] text-dim">0</span>
        </div>
      </div>
