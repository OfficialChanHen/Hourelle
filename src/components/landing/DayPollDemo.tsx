...
const label = (n: number) => `${DOW[n % 7]} ${DAYS[n].month} ${DAYS[n].day}`
        ...
        <div className="relative select-none p-4 sm:p-5">
          {near && (
            <>
              ...
              {/* the gap leaves room for the ring round each best day to stand on its own */}
              <div ref={grid} className="mt-3 grid grid-cols-7 gap-2 sm:gap-2.5">
                {DOW.map((d) => <span key={d} className="text-center text-[11px] font-semibold uppercase tracking-[.06em] text-dim"><span className="sm:hidden">{d[0]}</span><span className="hidden sm:inline">{d}</span></span>)}
                {DAYS.map((d) => {
                  const isMine = mine.has(d.n), n = OTHER_COUNTS[d.n]
                  const inBest = !!best && d.n >= best.s && d.n <= best.e
                  return (
                    <button
                      key={d.n} type="button" data-day={d.n} onClick={() => tap(d.n)} aria-pressed={isMine} disabled={!done} aria-label={label(d.n)}
                      className={`flex h-11 flex-col items-center justify-center rounded-[7px] text-[13px] font-semibold transition-colors duration-150 sm:h-12 ${isMine ? 'text-on-accent' : 'text-text'} ${inBest ? 'ring-2 ring-ochre ring-offset-2 ring-offset-s1' : isMine && pressing ? 'ring-2 ring-accent/60 ring-inset' : ''} ${done ? 'cursor-pointer' : 'cursor-default'}`}
                      style={isMine ? { background: 'var(--accent)' } : { background: heatOf(n), opacity: n === 0 ? 1 : 0.65 }}
                    >
                      <span className="leading-none">{d.day}</span>
                      <span className={`mt-0.5 text-[9.5px] font-medium leading-none ${isMine ? 'opacity-80' : 'text-dim'}`}>{count(d.n) ? `${count(d.n)} free` : ''}</span>
                    </button>
                  )
                })}
              </div>
